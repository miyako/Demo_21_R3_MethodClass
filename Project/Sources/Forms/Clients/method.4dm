...
Case of 
	: ($event=On Load)
		...
		Form.dataClass:=ds.CLIENTS
		Form.dataClassName:="CLIENTS"  //To be replaced by :=$dataClass.getInfo().name  //The Table (DataClass) name
		Form.dataClassPtr:=->[CLIENTS]  //To be replaced when it will be possible to get a pointer on a DataClass...
		Form.objectsNames:=New collection("Optional_Data"; "Numbers"; "eAddresses")  //Names ob Object Fields containing Multiple Values
		...
		For each ($listBox; Form.objectsNames)
			Settings_ApplyToListBox(Form.settings; "_LB_"+$listBox; True)
		End for each 
...
If (($event=On Load) | ($event=On Selection Change) | ($event=On Clicked) | ($event=On Drop) | ($event=On Data Change) | ($event=On After Edit))
	...
	$what:=OBJECT Get pointer(Object named; "_MORE_")->
	OBJECT SET VISIBLE(*; "@_1_@"; ($what=0))
	OBJECT SET VISIBLE(*; "@_2_@"; ($what=1))
End if 
