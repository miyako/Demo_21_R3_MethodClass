...
var $zePaths; $typeList; $propertyCol : Collection
	...
	If ($property#"_@")  //Fields beginning by "_" are supposed to be invisible
		Case of 
			: ($zeProperty.kind="storage")
				Case of 
						...
						For each ($path; $zePaths)
							Case of 
								: (($path="@[]") & ($what2do="SORT"))  //This is an array, and can't be used for sorting
									If (False)
										...
										If ($path="@.length")  //This the number of elements of an array
											$propertyCol.push(New object("name"; $zeProperty.name+"."+$path; "type"; "long"; "localName"; Util_Get_LocalizedMessage($zeProperty.name)+"."+$path))
										End if 
									End if 
								...
								: ($path="@[]")
									...
									$propertyCol.push(New object("name"; $zeProperty.name+"."+$path; "type"; "array"; "localName"; Util_Get_LocalizedMessage($zeProperty.name)+"."+$path))
								...
								Else 
									$propertyCol.push(New object("name"; $zeProperty.name+"."+$path; "type"; "variant"; "localName"; Util_Get_LocalizedMessage($zeProperty.name)+"."+$path))
					...
					: ($colOK2Use.indexOf($zeProperty.type)>-1)  //We can use it for sorting or searching
						$propertyCol.push(New object("name"; $zeProperty.name; "type"; $zeProperty.type; "localName"; Util_Get_LocalizedMessage($zeProperty.name)))
			...
			: ($zeProperty.kind="relatedEntities")  //We are going to get the related Many table's fields
				...
				Else 
					$propertyCol.push(New object("name"; $zeProperty.name; "type"; "1toN"; "localName"; Util_Get_LocalizedMessage($zeProperty.name)))  //$zeRelatedDataclass.getInfo().name gives the name of the dataclass
					For each ($relatedProperty; $zeRelatedDataclass)
						...
						If ($zeRelatedProperty.kind="storage")
							$propertyCol.push(New object("name"; $zeProperty.name+"."+$zeRelatedProperty.name; "type"; $zeRelatedProperty.type; "localName"; Util_Get_LocalizedMessage($zeProperty.name)+"."+Util_Get_LocalizedMessage($zeRelatedProperty.name)))
						End if 
					End for each 
				End if 
